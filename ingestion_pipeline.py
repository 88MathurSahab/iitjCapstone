...
def prepare_data_for_db(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_db["datetime_utc"].dt.tz is None:
        df_db["datetime_utc"] = pd.to_datetime(df_db["datetime_utc"]).dt.tz_localize("UTC")
    else:
...
def ingest_data_batch(
    ...
    """
    logger.info("Starting batch ingestion into %s (batch size: %d)", table_name, batch_size)
        ...
        with pyodbc.connect(connection_string, timeout=60) as conn:
            ...
            for i in range(0, total_rows, batch_size):
                ...
                try:
                    ...
                    logger.debug("Inserted batch %d-%d (%d rows)", i, i + len(batch), len(batch))
                except Exception as exc:
                    logger.error("Error inserting batch %d-%d: %s", i, i + len(batch), exc)
                    conn.rollback()
        ...
        with pyodbc.connect(connection_string, timeout=60) as conn:
            ...
            upsert_query = f"""
                ...
                WHEN NOT MATCHED THEN
                    INSERT ({column_names}) VALUES ({column_names});
            """
            ...
            for i in range(0, total_rows, batch_size):
                ...
                for _, row in batch.iterrows():
                    ...
                    values.append(tuple(row_values + row_values))
                ...
                try:
                    ...
                    logger.error("Error upserting batch %d-%d: %s", i, i + len(batch), exc)
                    conn.rollback()
